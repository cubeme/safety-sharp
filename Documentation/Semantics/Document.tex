...
component C
{
	...
	component D
	{
		...
		// provided port
		X(int p1, int p2; bool r1, bool r2)
		{
			Y(;);
			r1 = false;
			r2 = b;
	...
	fault F
	{
		...
		step
		{
			(true (* $\guardedArrow$ *) F := enabled; true (* $\guardedArrow$ *) F := disabled)
		}
	}
	...
	// required port
	M(in: int p1, int p2, inout: bool r1, bool r2);
		
	// provided port
	[NoFaults]
	X(in: int p1, int p2, inout: bool r1, bool r2)
	{
		locals
		{
			int x;
...
A \textit{fault} defines the the occurrence behavior of undesired or erroneous component behavior. Faults affect provided ports
such that the behavior of the model with faults is a conservative extension of the model without the occurrence of any faults. A
fault has to define at least the Boolean field \texttt{faulted}. It can optionally have further fields and a step behavior.
\begin{equation*}
	\faultDecl \in \FaultDecl := \faultDeclSyn{\fault}{\toListNE{\field}}{\toList{\varDecl}}{\stmFm}
\end{equation*} 

The fields declared by a fault can be read by the component only within the context of fault statements. The fault, on the
other hand, is free to read all component fields. The component has to explicitly invoke the fault's step function in the
